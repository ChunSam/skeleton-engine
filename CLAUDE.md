...
> Version v1.6.83 | package `skeleton-engine` v0.34.0, library crate `engine` | wgpu-based Rust 2D game engine (wgpu 29, MSRV 1.95, CI pin Rust 1.95.0) | **Cargo workspace** (members `.` + `engine_reflect_derive` proc-macro)  
...
| UI (UiNode, Button, Label, TextInput, ScrollView, Panel, LayoutSystem, UiEvent [now `PartialEq`]); **keyboard focus** — `UiFocus` resource (`Option<Entity>`, auto-inserted) + `UiSystem`'s focus pass (`src/ui/system/focus_pass.rs`): **Tab/Shift+Tab** cycle focus across focusable widgets (Button/TextInput/Slider/CheckBox, by entity index, skips hidden/disabled), draws a focus ring, **Enter/Space** activate (button click / checkbox toggle), **←/→** nudge a focused Slider, click also focuses; example `ui_focus`; `UiSystem`/`SteeringSystem` hold reused scratch buffers → construct via `::default()`/`::new()` | `src/ui/` |
