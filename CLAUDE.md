...
| **DialogueBox**, **DialogueChoice**, **DialogueSystem** (speaker + typewriter text box: per-line char reveal at `chars_per_sec`, two-stage `advance()` [press 1 completes line, press 2 next], optional `portrait`, UTF-8-safe; **localization** — `DialogueBox::localized(speaker_key, line_keys)` + `line_keys`/`speaker_key` + `resolve(&LocaleResource)` fill `lines`/`speaker` from translation keys WITHOUT touching reveal state, re-resolved each frame by `DialogueSystem` so a live `set_locale` retranslates mid-conversation; **branching** — `choices: Vec<(line, Vec<DialogueChoice>)>` via `with_choices`, `pending_choices()`/`choose(i)` jump to a choice's `goto` line [out-of-range goto clamps to end], `advance()` no-ops while a decision is pending; `DialogueSystem` ticks via `query_mut` + renders text-only screen-space via `TextQueue` [input-agnostic — game calls `advance`/`choose`], drawing the numbered choice list in place of the ▼ hint; examples `dialogue_demo` (linear), `dialogue_branching` (localized + branching, live en↔ko toggle)) | `src/dialogue.rs` |
