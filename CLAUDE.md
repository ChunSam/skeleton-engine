...
> Version v1.6.65 | package `skeleton-engine` v0.17.0, library crate `engine` | wgpu-based Rust 2D game engine (wgpu 29, MSRV 1.95, CI pin Rust 1.95.0) | **Cargo workspace** (members `.` + `engine_reflect_derive` proc-macro)  
...
| **DialogueBox**, **DialogueSystem** (speaker + typewriter text box: per-line char reveal at `chars_per_sec`, two-stage `advance()` [press 1 completes line, press 2 next], optional `portrait`, UTF-8-safe; `DialogueSystem` ticks via `query_mut` + renders text-only screen-space via `TextQueue` [input-agnostic — game calls `advance`]; example `dialogue_demo`) | `src/dialogue.rs` |
...
| ParticleEmitter (+ **`gravity: Vec2`** per-particle accel + **`emit_shape: EmitShape`** Point/Circle/Ring/Box spawn scatter — `with_gravity`/`with_emit_shape`; additive, ZERO/Point = prior behavior; example `particles_showcase`), ParticleSystem, ParticleBurst (one-shot burst + `ParticleEmitter::burst()`); ParticleConfigSet/ParticleConfigRegistry/ClipSetError-style ParticleConfigError (data-driven emitter configs from RON; `App::load_particle_configs` = registry + DataTable-style hot-reload; gravity/emit_shape not yet in RON or `GpuParticleEmitter` — follow-up) | `src/particle/` (`mod.rs`, `config_set.rs`) |
