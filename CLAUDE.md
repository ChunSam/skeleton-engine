...
> Version v1.6.15 | package `skeleton-engine` v8.10.0, library crate `engine` | wgpu-based Rust 2D game engine (wgpu 29, MSRV 1.92, CI pin Rust 1.95.0) | **Cargo workspace** (members `.` + `engine_reflect_derive` proc-macro)  
...
| In-game editor: F1 overlay + F2 **docked mode** (dock layout, viewport input gate, engine-level editor pause, gizmo + screen-space UI gizmo / 8-handle resize, scene save, Data Tables panel) | `src/app/editor/` |
