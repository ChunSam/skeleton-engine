...
> Version v1.6.86 | package `skeleton-engine` v0.37.0, library crate `engine` | wgpu-based Rust 2D game engine (wgpu 29, MSRV 1.95, CI pin Rust 1.95.0) | **Cargo workspace** (members `.` + `engine_reflect_derive` proc-macro)  
...
| UI (UiNode, Button, Label, TextInput, ScrollView, Panel, LayoutSystem, UiEvent [now `PartialEq`]); **keyboard focus** — `UiFocus` resource (`Option<Entity>`, auto-inserted) + `UiSystem`'s focus pass (`src/ui/system/focus_pass.rs`): **Tab/Shift+Tab** (or gamepad **D-pad Down/Up**) cycle focus across focusable widgets (Button/TextInput/Slider/CheckBox, by entity index, skips hidden/disabled), draws a focus ring, **Enter/Space** (or gamepad **A**/South) activate (button click / checkbox toggle), **←/→** (or D-pad Left/Right) nudge a focused Slider, click also focuses; gamepad nav is folded into `InputSnapshot` from the first connected `GamepadState` pad (`src/ui/system/state.rs`, optional — no pad = no-op; analog stick not used, D-pad only); example `ui_focus`; `UiSystem`/`SteeringSystem` hold reused scratch buffers → construct via `::default()`/`::new()` | `src/ui/` |
