...
> Version v1.6.73 | package `skeleton-engine` v0.24.0, library crate `engine` | wgpu-based Rust 2D game engine (wgpu 29, MSRV 1.95, CI pin Rust 1.95.0) | **Cargo workspace** (members `.` + `engine_reflect_derive` proc-macro)  
...
| UI (UiNode, Button, Label, TextInput, ScrollView, Panel, LayoutSystem, UiEvent); `UiSystem`/`SteeringSystem` hold reused scratch buffers → construct via `::default()`/`::new()` (v10, no longer unit structs) | `src/ui/` |
