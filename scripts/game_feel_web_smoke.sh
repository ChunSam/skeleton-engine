#!/usr/bin/env bash
# wasm render smoke for the `game_feel` example (juice toolkit + settings menu capstone).
#
...
# renders a wasm frame and looks at it. This builds `game_feel` to wasm, serves it,
# renders one frame headless on a simulated Retina (DPR=2) display, asserts the
...
# the game-feel behaviors themselves (jump forgiveness, hit-stop, trail) — those are
# interactive; play the web page (or the native example) to check them. For the
# frame, EYEBALL the screenshot: two grey pads with a gap, three red dummies, the
# yellow player, and the HUD lines should all be visible.
#
...
#
# Usage:
#   scripts/game_feel_web_smoke.sh                 # build + render + assert
#   CHROME=/path/to/chrome scripts/game_feel_web_smoke.sh
#   SMOKE_KEEP=1 scripts/game_feel_web_smoke.sh    # keep the screenshot
#
...
WEB_DIR="$PROJECT_ROOT/examples/game_feel/web"

PORT="${SMOKE_PORT:-8087}"                 # static-file server port (configurable)
SHOT="${SMOKE_SHOT:-/tmp/game_feel_web_smoke.png}"
...
PROFILE="$(mktemp -d -t chrome_gf_smoke.XXXXXX)"
HTTPD_PID=""
cleanup() {
...
echo ">>> [1/4] building game_feel -> wasm (release)..."
...
# run_game_feel() without the (human-only) Start click. Run Chrome backgrounded:
...
"$CHROME" --headless=new \
  ...
  --force-device-scale-factor=2 --window-size=900,540 \
...
if (( fail )); then
  echo ">>> GAME_FEEL WEB SMOKE: FAIL" >&2
  echo "    screenshot : $SHOT" >&2
  exit 1
fi
echo ">>> GAME_FEEL WEB SMOKE: PASS (run + non-blank render)"
echo "    eyeball $SHOT — pads + gap, three dummies, the player, and the HUD lines should show"
