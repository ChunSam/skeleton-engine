#!/usr/bin/env bash
# wasm render smoke for the `embedded_image` example (`App::load_image_bytes`).
#
...
# `load_image_bytes` exists so an image can ship INSIDE the .wasm module, and "it
# compiles for wasm32" does not prove the sprite actually draws in a browser. That
# gap was real for this example: it was silently unbuildable for wasm32 from the day
# it was added until v0.136.0, because nothing built it for that target. This builds
# the example to wasm, serves it, renders one frame headless on a simulated Retina
# (DPR=2) display, and asserts both halves of the claim.
#
# The two assertions:
#   1. NO image file is served beside the page. The whole point is that the PNG is
...
#   2. The rendered frame is non-blank, so the sprite really did decode from the
#      embedded bytes and reach the GPU.
#   Together those two say: the sprite rendered, and it cannot have come from a file.
#
# What this does NOT catch: a WRONG image (or the white fallback texture, the failure
# the verbatim-key invariant exists to prevent) still yields a non-blank frame, because
# the HUD text alone clears the byte threshold. For that, EYEBALL the saved screenshot —
# it should show the 32x32 sprite scaled up on the left, NOT a white square, plus a red
# `App::asset_failures()` panel naming the deliberate corrupt embed.
#
# The atlas twin is scripts/embedded_atlas_smoke.sh; keep the two in step.
#
...
#
# Usage:
#   scripts/embedded_image_smoke.sh                 # build + render + assert
#   CHROME=/path/to/chrome scripts/embedded_image_smoke.sh
#   SMOKE_KEEP=1 scripts/embedded_image_smoke.sh    # keep the screenshot
#
...
WEB_DIR="$PROJECT_ROOT/examples/embedded_image/web"

PORT="${SMOKE_PORT:-8091}"                 # static-file server port (configurable)
SHOT="${SMOKE_SHOT:-/tmp/embedded_image_smoke.png}"
# Threshold measured on this page, not inherited: an engine-never-drew frame (the
# same URL WITHOUT ?autostart=1, so only the DOM paints) is 5,582 bytes, and a real
# rendered frame is 84,639. 15,000 sits between them with room on both sides.
MIN_PNG_BYTES="${SMOKE_MIN_BYTES:-15000}"
...
PROFILE="$(mktemp -d -t chrome_ei_smoke.XXXXXX)"
HTTPD_PID=""
cleanup() {
...
echo ">>> [1/5] building embedded_image -> wasm (release)..."
...
if [[ -n "$strays" ]]; then
  echo "  FAIL: an image file is served beside the page — the embedded-image claim is void:" >&2
  echo "$strays" | sed 's/^/         /' >&2
  exit 1
fi
echo "  ok  : no image file under $WEB_DIR -> a rendered sprite CANNOT have been fetched"
...
# run_embedded_image() without the (human-only) Start click. Run Chrome backgrounded:
...
"$CHROME" --headless=new \
  ...
  --force-device-scale-factor=2 --window-size=720,420 \
...
if (( fail )); then
  echo ">>> EMBEDDED_IMAGE SMOKE: FAIL" >&2
  echo "    screenshot : $SHOT" >&2
  exit 1
fi
echo ">>> EMBEDDED_IMAGE SMOKE: PASS (embedded image rendered on the web, with nothing to fetch)"
echo "    eyeball $SHOT — the 32x32 sprite scaled up (NOT a white square), plus the red asset_failures() panel"
