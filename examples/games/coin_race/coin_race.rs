...
//!
//! # Terminals 2, 3, ... — one window per player
...
fn main() {
    use engine::{
        ...
        Scene, Sprite, System, TextQueue, Transform, WindowConfig, World,
    };
    use glam::Vec2;
    ...
    impl CoinRaceSystem {
        ...
        fn ensure_coin(&mut self, world: &mut World, coin: usize, x: f32, y: f32) {
            ...
            self.coins.entry(coin).or_insert_with(|| {
                Self::spawn_square(world, pos, COIN_SIZE, -1.0, [1.0, 0.84, 0.2])
            });
        }
    ...
    fn read_axis(world: &World) -> (f32, f32) {
        ...
        let right =
            (input.is_pressed(KeyCode::KeyD) || input.is_pressed(KeyCode::ArrowRight)) as i32;
