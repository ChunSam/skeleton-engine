//! hex_autotile — autotiling on a **hexagonal** tilemap (`Neighborhood::Hex6`).
//!
//! Autotile masks are computed from the `tiles[row][col]` grid, so they work on any projection once
//! the neighborhood matches the layout. This pairs `TilemapProjection::Hexagonal` (pointy-top,
//! odd-r) with `Neighborhood::Hex6` (the 6 parity-aware hex neighbors). A simple **interior vs
//! edge** rule shows the mask driving tile choice: a hex with all 6 neighbors filled (mask 63) draws
//! grass; any hex with an open edge draws sand. Dig holes and the rim around them turns to sand —
//! recomputed reactively by `TilemapSystem`.
//!
//! (Isometric maps autotile too, with the square `Edge4`/`Blob8` neighborhoods — iso is the same
//! grid topology as orthographic, just rendered as diamonds; see `iso_tilemap` + the unit tests.)
//!
//! Run from the repo root:  `cargo run --example hex_autotile`
    ...
    fn run(&mut self, world: &mut World, _dt: f32) {
        ...
        if let Some(tq) = world.resource_mut::<TextQueue>() {
            tq.push(DrawText::new(
                "Hex autotile (Hex6) — Arrows: move · Space: dig/fill · ESC: quit",
                Vec2::new(20.0, 20.0),
                18.0,
...
fn main() {
    ...
    app.world.insert_resource(WindowConfig {
        title: "hex_autotile — Hex6 interior/edge autotiling".to_string(),
        width: 900,
        height: 600,
        clear_color: [0.06, 0.07, 0.10, 1.0],
    });

    // Reuse the pointy-top hex atlas: tile 0 = grass (interior), tile 1 = sand (edge).
    app.load_image("examples/assets/hex_tiles.png");
    let atlas = TilemapAtlas::new("examples/assets/hex_tiles.png", 2, 1);

    // Interior/edge rule: only mask 63 (all 6 hex neighbors filled) shows grass; every other mask
    // (any open edge) shows sand. A 2-tile autotile driven entirely by the Hex6 neighbor mask.
