//! hex_tilemap — a **hexagonal** tilemap (`TilemapProjection::Hexagonal`).
//!
//! Same `Tilemap` + `TilemapSystem` as the square/iso examples, built with
//! `.with_projection(TilemapProjection::Hexagonal)` — a pointy-top hex grid in odd-r offset
//! coordinates (odd rows shifted right by half a tile). Demonstrates both directions:
...
//!
//! Run from the repo root:  `cargo run --example hex_tilemap`
...
/// Moves the selection over the hex grid and reports the mouse-picked cell.
struct HexControl {
    map: engine::Entity,
    sel: (usize, usize),
}
...
impl System for HexControl {
    fn run(&mut self, world: &mut World, _dt: f32) {
        ...
        // Pick the hovered hex from the cursor's world position (pixel → axial → cube-round).
        let hover = world
            .resource::<Camera>()
        ...
        if let Some(tq) = world.resource_mut::<TextQueue>() {
            tq.push(DrawText::new(
                "Hexagonal tilemap — Arrows: move selection · mouse: hover-pick · ESC: quit",
                Vec2::new(20.0, 20.0),
                18.0,
...
fn main() {
    ...
    app.world.insert_resource(WindowConfig {
        title: "hex_tilemap — hexagonal projection".to_string(),
        width: 900,
        height: 600,
        clear_color: [0.06, 0.07, 0.10, 1.0],
    });

    // 2-tile atlas (2 cols × 1 row): tile 0 = grass hex, tile 1 = sand hex.
    app.load_image("examples/assets/hex_tiles.png");
    let atlas = TilemapAtlas::new("examples/assets/hex_tiles.png", 2, 1);

    // All grass with a sand border so the hex layout + odd-row offset are obvious.
    ...
    // origin = cell (0,0)'s center. tile_size = hex flat-to-flat width.
