#!/usr/bin/env bash
# Build the embedded_image example to wasm and serve it in the browser.
#
...
#
# What this demo proves: the PNG is inside the .wasm module itself
# (`include_bytes!` → `App::load_image_bytes`), so the page renders the sprite
# with NO image fetch — the thing a path-loaded image cannot do on the web
# without async plumbing. There is no `assets/` directory served beside this page.
#
# The atlas twin is examples/embedded_atlas/web/build.sh; keep the two in step.
#
# Prerequisites:
...
echo ">>> building embedded_image → wasm (release)..."
cargo build --release --example embedded_image --target wasm32-unknown-unknown

echo ">>> generating web bindings (wasm-bindgen --target web)..."
wasm-bindgen --target web --out-dir "$SCRIPT_DIR/pkg" \
  target/wasm32-unknown-unknown/release/examples/embedded_image.wasm
