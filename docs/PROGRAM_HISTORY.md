# Playable-examples program — history

> **A closed record, not a backlog.** This is the completed candidate **A–O** dogfooding program
> (2026-05-29 → 2026-06-15) and the release/hardening follow-ups of that era. It was moved out of
> `docs/NEXT_WORK.md` on 2026-08-03, where it had grown to 84% of a file named *Next* Work.
>
> Every entry here is **done**. The rule the program ran under — *a feature is not done until a
> small, playable example game in `examples/` exercises it in real play* — still governs new work,
> but it lives in `CLAUDE.md` and `docs/VISION.md`, not here.
>
> Release-facing narrative is `docs/CHANGELOG.md`. What this file keeps that the changelog does not
> is **why each candidate was chosen and what it surfaced** — the engine gaps only real play found.
