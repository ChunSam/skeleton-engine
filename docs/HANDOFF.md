...
| ~~Coin race on wasm + v4.0.0 tag (v4.1.0, 2026-06-08)~~ | ~~`coin_race_game` now also runs in the browser: a `#[wasm_bindgen] run_coin_race` entry **in the example** (engine lib unchanged) + `examples/games/coin_race/web/` (`index.html` + `build.sh` driving `cargo build --example` + `wasm-bindgen`). Establishes the reusable "ship an engine *example* to the web" path — the lib-only `wasm-pack` demo (`examples/wasm/`) cannot build examples. Gotcha: `wasm-bindgen` fails with a `clone_ref` intrinsics error on an **empty** example `main` (DCE strips the bindgen intrinsics); a real entry that exercises the engine fixes it. Only engine-source change = wasm-only `format!` arg-inlining in `async_loading.rs`/`save.rs` (clippy-on-wasm clean). Also git-**tagged v4.0.0** (the repo sat at package 4.0.0 but untagged) and reconstructed the missing 3.0.0+4.0.0 `docs/CHANGELOG.md` sections. Verified: browser tab's wasm WebSocket connects to the native authoritative server + renders player and server-spawned coins via WebGL2 (headless-Chrome shot); full `+1.88.0` gate green (311 lib + 5 server tests).~~ | — | done |
