...
- `SpriteRenderer` caches file textures under both the original requested path and the canonical handle path. After `App::load_image()` with a relative path, `Sprite::textured_with_handle(...)`, `DrawImage::textured_with_handle(...)`, and `AtlasSprite` do not fall back to white even when prioritizing the handle path.
...
| ~~Coin race playable example (candidate M, 2026-06-08)~~ | ~~`examples/games/coin_race/` (`coin_race_game` + `coin_race_server`): first **playable** multiplayer game (goal + win condition), closing networking — the last never-in-a-game subsystem (previously only the `mp_server`/`mp_client` position-relay demos). The server is **authoritative** (owns the coin field + scoreboard) so contested pickups resolve correctly. No engine gap forced a change: `NetworkClient`/`NetworkSystem`/`NetworkEvent` carried it as-is; native `Disconnected` covers server-down detection, so `is_connected()` (wasm-only) stayed unforced per the "fix only the gap the example hits" bar. Deferred: a reusable remote-entity (`HashMap<id,Entity>`) bookkeeping helper (two examples = not enough signal). Verified end-to-end via a throwaway tungstenite probe (authority/relay/lifecycle) + a 2-window render screenshot; server has 5 unit tests.~~ | — | done |
| ~~Coin race on wasm + v4.0.0 tag (v4.1.0, 2026-06-08)~~ | ~~`coin_race_game` now also runs in the browser: a `#[wasm_bindgen] run_coin_race` entry **in the example** (not the engine lib) + `examples/games/coin_race/web/` (`index.html` + `build.sh` driving `cargo build --example` + `wasm-bindgen`). Establishes the reusable "ship an engine *example* to the web" path — the lib-only `wasm-pack` demo (`examples/wasm/`) cannot build examples. Gotcha: `wasm-bindgen` fails with a `clone_ref` intrinsics error on an **empty** example `main` (DCE strips the bindgen intrinsics); a real entry that exercises the engine fixes it. Engine-source changes: wasm-only `format!` arg-inlining (`async_loading.rs`/`save.rs`) + a **HiDPI viewport fix** (`schedule.rs`) found by the Retina playtest — `ViewportSize` was `surface/DPR` for all targets, but the wasm surface is already CSS-logical, so on Retina (DPR 2) the world viewport was halved and fixed-coordinate scenes rendered off-screen (only DPR 1 worked); the DPR division is now native-only. Also git-**tagged v4.0.0** (the repo sat at package 4.0.0 but untagged) and reconstructed the missing 3.0.0+4.0.0 `docs/CHANGELOG.md` sections. Verified: browser wasm WebSocket connects to the native server + the field renders correctly **on a real Retina display** (player + coins; pre-fix only DPR 1 rendered) + the player moves under held keys; deterministic DPR=1-vs-DPR=2 headless repro confirmed the fix; full `+1.88.0` gate green (311 lib + 5 server tests).~~ | — | done |
| ~~v4.1.0 finalize + repo-wide Korean→English (networking-dogfood seq 2, 2026-06-09)~~ | ~~Release hygiene + wasm hardening + docs. **Moved the `v4.1.0` tag** from `7c6f9c0` (pre-fix) to `ebd9081` so the tagged wasm `coin_race` includes the default-font + canvas-size fixes (all 3 wasm fixes are wasm-only/additive → native consumers unaffected); force-pushed and pushed the rust-survivors v4.1.0 pin (`e6176fa`). **Re-verified `examples/wasm` `run_demo`** renders correctly on headless Retina (DPR=2) — squares spread across the full 1280×720 viewport + the embedded-DejaVu HUD text, no clipping; no engine change needed (its first wasm text render, exercising the `DEFAULT_FONT` fallback). **Added `scripts/wasm_smoke.sh`** — an optional local headless-Chrome (DPR=2) render+network smoke check: asserts the wasm app connects to the server AND renders a non-blank frame, saving the screenshot. It catches the *catastrophic* class (no-run / no-connect / blank) that CI's wasm-*build* misses, but NOT the subtle 3-bug class — empirically each reverted fix yields a wrong-but-NON-blank frame (the HiDPI-reverted shot is even *larger*), so no cheap scalar separates them; eyeball the saved shot for those. Not a CI gate (needs Chrome + a matching `wasm-bindgen-cli`). **Translated all Korean → English repo-wide**: remaining example comments/display strings + **~110 `src/` files** (~2285 comment lines + developer-facing log/panic/expect/assert diagnostic strings), via 10 parallel Sonnet subagents. Intentional Korean DATA is preserved (test/locale fixtures, not prose): the `"ko"` locale values in `locale.rs`/`ui::localized`, and the Hangul/IME fixtures in `text_input.rs`/`input::state`/`renderer::text` (`"안녕"`). Caught a cross-file coupling a parallel split could have broken — `app.rs` `#[should_panic(expected=…)]` ↔ a `panic!` message in `app/schedule.rs` — and kept them in sync. Full `+1.88.0` gate green (311 lib + 5 server + 3; fmt re-applied to wrap the now-longer assert messages).~~ | — | done |
| ~~wasm Retina crispness (networking-dogfood seq 3, 2026-06-09)~~ | ~~Re-enabled the native HiDPI render model on wasm. The wasm drawing buffer was logical-size (the seq-1 HiDPI fix forced `scale_factor = 1`), so on Retina the browser upscaled a half-resolution canvas — correct layout, but soft. Now the buffer = logical × `devicePixelRatio` (uniform scale, capped so neither axis exceeds the WebGL2 2048 limit) while the canvas CSS *display* box stays logical, so the browser maps the buffer 1:1 (crisp). `ViewportSize` stays logical (`= gpu.config / scale`) and `DisplayScaleFactor = buffer / logical`, so sprites/UI keep their coordinates and glyphon text renders at device resolution. **Logical size = the authored `<canvas>` width/height attributes** (stored in `WASM_LOGICAL_SIZE`), **not `WindowConfig`** — a scene transition (`World` reset) reverts `WindowConfig` to its default (the first attempt used `WindowConfig` and coin_race rendered as a 1280 viewport in an 800 canvas). Edits: `src/app/window.rs` (`finish_init` sizes the buffer + locks CSS + resizes; `Resized` recomputes from `WASM_LOGICAL_SIZE`), `src/app/schedule.rs` (render scale = buffer/logical), `src/app.rs` (the thread-local). Verified on a real-GPU Retina browser (Chrome Metal/ANGLE, not just SwiftShader) + headless DPR=2: coin_race (800→1600 buffer) and run_demo (1280→2048, clamp path, aspect preserved) render correctly and crisp; `scripts/wasm_smoke.sh` + full `+1.88.0` gate green (311 lib).~~ | — | done |
| ~~RemoteEntities helper — minimal slice (networking-dogfood seq 3 Phase 2, 2026-06-09)~~ | ~~Extracted the `HashMap<id, Entity>` remote-entity lifecycle that every networked game reimplemented inline into `engine::RemoteEntities<K>` (`src/network.rs`): `get_or_spawn` / `get` / `contains_key` / `remove` (despawns) / `clear` / `len` / `is_empty` / `iter`. Migrated `mp_client` (`remote_players`) and `coin_race` (`remote_players` + `coins`) onto it — behavior identical. **Deliberately the lifecycle slice only**: what to spawn (closure), how to update (game calls `get` + mutates via `World`), and parallel game-state maps (`coin_pos`/`scores`/`claimed`) stay in the game. A *richer* version (interpolation, prediction/reconciliation, update callbacks, staleness) is **deferred** — the two call sites are too similar to reveal the shape, and a wrong public API on a skeleton is worse than the duplication removed; revisit with a 3rd distinct networked example (open questions + candidates in `docs/REMOTE_ENTITIES_DESIGN.md`). Doctest + 2 unit tests; full `+1.88.0` gate green (**313 lib** + 35 doctests); `wasm_smoke.sh` confirms coin_race still renders + connects through the helper.~~ | — | done |
| ~~Client-prediction shooter (networking-dogfood seq 4, 2026-06-09)~~ | ~~`examples/games/predict_shooter/` — the 3rd distinct networked example: an authoritative fixed-tick server (`predict_shooter_server`: 60 Hz sim, per-client seq'd input queue, bullets, 30 Hz snapshots with per-client `ack`) + a client demonstrating the three pillars of responsive netcode — **client-side prediction** (input moves you immediately), **server reconciliation** (snap to the acked state + replay un-acked inputs, no drift), and **remote interpolation** (other players + bullets rendered ~100 ms in the past, lerped). Pure netcode in `client_net.rs` (`Prediction`, `Interp`) with 7 server + 6 client unit tests; remote entities use `engine::RemoteEntities`. Ships native + to the browser (`web/` harness, mirroring coin_race). **Real-play verified** (2 native clients connect, see each other consistently, input → prediction + remote interpolation) + headless wasm smoke. **RemoteEntities design decision:** interpolation is orthogonal to the lifecycle map → stays a separate, example-local concern; the v4.3.0 minimal `RemoteEntities` API is unchanged (see `docs/REMOTE_ENTITIES_DESIGN.md`). No public API change → no version bump.~~ | — | done |
| ~~Phase-3 polish: lint + Camera bounds + InputMap gamepad + shooter feel tuner (networking-dogfood seq 5 follow-ups, 2026-06-09)~~ | ~~Four-part session; the two engine features were built by **parallel worktree subagents** (Sonnet), with integration + lint + tuner in the main session. **(1) Stable-lint:** `(i+j) % 2 == 0` → `((i+j) as u32).is_multiple_of(2)` in `minimap`/`touch_demo`/`split_screen` so local-stable-1.95 `verify.sh` is green — cast to `u32` because signed `is_multiple_of` is **not** stable on the 1.88.0 CI pin (parity preserved: 2³² is even). **(2) Camera world-bounds:** added `Camera::bounds: Option<(Vec2,Vec2)>` + `clamp_to_bounds(vw,vh)`; App clamps each frame right after `Camera::update` (reads `ViewportSize`), so follow cameras need no code; small-world rule pins to `bounds.min`; `lit_dungeon` dropped its hand-rolled `CameraFollowSystem` clamp; 6 unit tests. **(3) InputMap gamepad:** additive `bind_gamepad_button`/`bind_gamepad_axis` (+ `AxisBinding`, re-exported `engine::AxisBinding`) + `is_pressed_with_gamepad`/`just_pressed_with_gamepad`/`just_released_with_gamepad` resolution ORing keyboard+gamepad; keyboard-only methods unchanged (generic bound widened to `A: Eq+Hash+Clone`, the lone semver-relevant detail — backward-compatible for any `Clone` action enum); `bind` is now additive multi-key; `survivor_game` drives every action from keys OR a controller (DPad+sticks); 12 unit tests. **(4) predict_shooter feel tuner:** `INTERP_DELAY` (the one feel parameter automation can't judge: smoothness vs. latency; default 100 ms ≈ 3× the 33 ms snapshot interval) made a runtime field tunable with `[`/`]` (±10 ms, clamped 0–300) + a live HUD readout, so it is A/B-able in real play — validated 100→160→40 ms (subjective value is the user's call; on localhost ~66 ms = 2× interval is likely snappier with no smoothness cost). All additive → **no version bump**. Combined `+1.88.0` gate green: fmt, clippy --all-targets, wasm lib build, **333 lib tests**, `RUSTDOCFLAGS=-D warnings` doc.~~ | — | done |

> **Current status**: post-v1.0 breadth-expansion. Vision (`docs/VISION.md`) = a forkable general-purpose 2D skeleton, features verified with small playable examples. Playable examples A–F shipped (platformer, scene-flow, maze-escape, sokoban, shooter, **survivor**), plus coverage follow-ups settings-menu, lit-dungeon, timeline-cutscene (L), and **coin-race (M)** — which closes networking, the last never-in-a-game subsystem, so **every engine subsystem now has at least one playable example**. Confirmed native·wasm32 builds (lib+bins), **311 lib tests + 5 coin_race_server tests** passing, 0 clippy warnings (`-D warnings`, all-targets). Engine git-tagged **v4.0.0**; **v4.1.0** = `coin_race` also runs in the browser (the first engine *example game* to ship to wasm — reusable `cargo build --example` + `wasm-bindgen` path), with the **`v4.1.0` tag now at `ebd9081`** (includes the default-font + canvas-size wasm fixes). The engine source is now **fully English** (all `src/` + example comments and developer-facing diagnostics translated; only deliberate Korean locale/IME test fixtures remain). Latest work: v4.1.0 tag finalize + rust-survivors pin push + `run_demo` Retina re-verify + `scripts/wasm_smoke.sh` + repo-wide Korean→English. Deferred follow-ups (planned, not breadth) are tracked in `plans/handoffs/PLAN_networking-dogfood_deferred-polish_2026-06-09.md`.
