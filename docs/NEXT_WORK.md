...
| **`EditorHistory` has no cap — MEASURED 2026-08-31, and the number is large enough to need a decision.** `push` appends forever; nothing trims. A cell is **24 B** as the row said, and `EditorCmd` itself is **208 B** (sized by its fattest variant, `DeleteEntity`'s inline `EntityDef`) which the row did not say. On a 256x256 tilemap one **Bucket fill is 1.50 MB** (65,536 cells, exactly — no capacity slack), so **10 fills = 15 MB and 50 = 75 MB**, retained until a scene reset. The other end is nothing: **1,000 freehand strokes = 70.75 KB** total, spine included. Four orders of magnitude between two ordinary editor gestures, which is the finding — **a cap counted in commands would be meaningless**; it has to be a byte budget that trims oldest, or nothing. ⚠️ **Now a decision, not a measurement**: a byte-budgeted trim makes undo silently stop going back, which is user-visible. Options — (a) trim oldest past a budget (64 MB?), (b) leave it and say so where `push` is defined, (c) cap only `PaintTiles` payload size. | `src/app/editor/history.rs` | The decision. The measurement is done and is above |
