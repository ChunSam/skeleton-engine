...
full read of an 11,202-line subsystem found and what each row cost to settle. ⚠️ One half-row
survives it: `VirtualJoystick` still cannot appear in a scene (see the decisions table). Previously:
...
maintainer decision). **Nothing in this section is open** except the `VirtualJoystick` half-row
noted above.
