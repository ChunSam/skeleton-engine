...
**Shipped so far: 36 of the 39, in seven batches** — v0.156.27 (the four panics + two blank-render
...
version bump), v0.156.31 (four read-derived behaviour rows), and v0.157.0 (the four allocation
rows — the first release in this program to break API). **What is left is 3 rows: 1 read-derived
behaviour, open on a decision rather than on doubt, and 2 that need the maintainer** — no
reproduced defect, no doc drift and no measured allocation remains in this section.
