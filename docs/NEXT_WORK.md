...
**Four items. Three are deliberately unscheduled** — each gated on a trigger, none on a decision —
**and the fourth is a live flake gated on nothing**, filed 2026-08-24 and the only row here that
costs something every day it stays open. A fifth
...
| **`NETPLAY_SELFTEST` check 7 is flaky** — the contested pickup | **Filed 2026-08-24, gated on nothing; this is the one row here that is not waiting for a trigger.** The check drives two clients onto the same pickups against a real `netplay_server` on an OS-assigned port, and intermittently the server grants nothing: `FAIL: two pilots sat on 2 pickup(s) together and the server granted none — scores 0 / 0`. ⚠️ **It is not new, and it was not caused by v0.154.3** — measured both ways on macOS debug builds: **3/16** failures with that release's logger installed, **1/8** with `logging::init()` commented out, and **1/8** with the logger on in a separate batch. Same rate, same failure, either side of the change. It surfaced only because it reddened `Test (native)` on #502, an unrelated PR. ⚠️ **`Test (native)` is a required context**, so every occurrence blocks a merge that has nothing to do with netplay — the cost is paid by whoever is unlucky, which is also why it went unnoticed while `main` stayed green for eight runs (the check landed 2026-08-21, so only the last few of those ran it at all). The check names its own two candidate causes and neither has been tested: the claim never reaching the server, or the server's distance validation refusing a legitimate one. ⚠️ **Do not settle it by loosening the assertion.** A contested pickup granting nothing is exactly the class of bug this game was rebuilt to catch, and check 7 is the only thing in the repo that drives two clients at once. What settles it: instrument the server's claim path and re-run the loop above — the reproduction is ~17% per run locally, so a batch of 16 is one command and about a minute. |
