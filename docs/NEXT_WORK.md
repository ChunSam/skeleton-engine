...
tests, each judged on whether it can fail on its stated cause), and `ui/docked/*` (all seven
files; its pass died to a usage limit and was re-run — its rows are the last eleven below).
⚠️ The other pass that died — `ui/mod.rs` + `shortcuts.rs` + `reparent.rs` + the four small
panels, ~1,300 lines — was also re-run and **its rows are not here yet**; they land when it
does, and until then those lines are the review's remaining debt.
...
Forty-five rows. The behaviour ones are ordered by what they cost a user; the test-quality ones
(a test that cannot fail on its stated cause) are one batch, the way v0.155.2 shipped three; the
doc ones are cheap and go with whichever fix touches the file. Every row names its instrument.
...
| **`finish_paint_stroke`'s doc says `paint_mode` survives both abandon sites ("re-selecting a tilemap resumes painting"); at the selection-ceasing-to-be-a-Tilemap site the caller clears it on the next line**, and `state.rs:330` says so. Only the selection→`None` site leaves it set. | `src/app/editor/ui/tile_paint.rs:377-378` vs `src/app/editor/ui/gizmo.rs:74-75` | A test per site (select a non-Tilemap, `update_editor_gizmo`, `assert!(!paint_mode)`; select `None`, assert it survives), then the sentence names the split. |
| **`anchor_base`'s doc says it is the single shared definition `UiNode::screen_pos` uses; `screen_pos` has its own copy** (`rg -n anchor_base src` → 4 hits, all in `gizmo_math.rs`), and the only test uses `anchor_base` on both sides. They are two *because* `anchor_base` is `cfg(not(wasm))` and `screen_pos` compiles on wasm — the doc claims the opposite. They match today. | `src/app/editor/ui/gizmo_math.rs:18-23` vs `src/ui/node.rs:145-158` | Unit test over all 7 anchors: `anchor_base(a, size, vw, vh) + offset == UiNode { anchor: a, size, offset }.screen_pos(&vp)`; then the doc says why there are two. |
| **Two wasm-branch comments say "log and return"; nothing logs.** The doc at `:29` ("no-op on wasm") is the truth — bar the registry insert + `register_persistent` at `:35-45`, which do run on wasm. | `src/app/editor/loading.rs:66`, `:168` | Eyes — a comment; nothing executable distinguishes it. |
