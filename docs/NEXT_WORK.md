...
Fifty-five rows. The behaviour ones are ordered by what they cost a user; the test-quality ones
...
| **Pasting a copied parent+child pair attaches the child copy to the *original* parent** — the tag lookup is first-wins and the original precedes the fresh spawn — so the pasted parent has no child. Known as a tag limitation on the delete-undo path (`history.rs:28-32`); unmentioned on the paste path. | `src/app/editor/ui/shortcuts.rs:112-132` → `src/prefab.rs:310-334` | Unit test: copy P+C, paste → `Parent(C′) == P′`. |
...
| **The cheatsheet says "Delete"; Backspace deletes too.** | `src/app/editor/ui/shortcuts.rs:39` vs `:220` | Reading; one row of text. |
