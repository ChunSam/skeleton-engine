...
**Two rows are left open, and both are about `resize`/format policy rather than about a frame:**
the bloom pipeline rebuild and `load_texture_with_format` ignoring `format` on a cache hit.
