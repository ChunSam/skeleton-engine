...
| **`EditorHistory::clear()`'s doc names a danger the ECS makes impossible.** It justifies clearing both stacks by "the ECS reuses entity ids, so a stale `DeleteEntity(e)` … resolves onto whatever NEW entity now occupies that slot". `Entity` is generation-checked (`src/ecs/world.rs:22`) and `despawn` bumps the generation (`src/ecs/world/entities.rs:69`), so a stale handle does not match. ⚠️ **The call is still correct** for a reason the comment does not give: after a reset the stack is a pile of silent no-ops that still drive `*selected = Some(<dead entity>)`. Do **not** "fix" this by deleting `clear()`. | `src/app/editor/history.rs:86` | Read — both sides already read |
| **Two consumers of the same rect disagree on the degenerate window.** Both do `central_rect.or_else(compute_central_rect)`. When both are `None` (window narrower than 561 logical pt, or shorter than 197) `render/docked.rs` skips the scene render while `schedule.rs` re-derives the subtraction **by hand** with `.max(1.0)` and publishes a **1×1** `ViewportSize`. Every system reading it runs against a viewport no frame matches. Reachable by dragging the window narrow; recoverable by dragging back, so it is a wrong state, not a stuck one. The formula living in two places is CLAUDE.md's *share the policy, not the implementation* on a second axis. | `src/app/schedule.rs:308`, `src/app/render/docked.rs:196` | A unit test — both arms are pure given `(w, h)`, no GPU |
| **Three comments say a migration is pending that has landed.** All three call `compute_central_rect` / `MARGIN_*` placeholders "until package 2 replaces them". Package 2 landed: `ui/docked/mod.rs:187` writes the real panel rect every docked frame. ⚠️ Not cosmetic — a reader concludes the margins are scaffolding to delete, but they are the **live first-frame fallback** (`window.rs:611` depends on that case). | `src/app/editor/docked_rt.rs:1`, `src/app/editor/state.rs:280`, `src/app/render/docked.rs:99` | Read (done) |
...
| **Deleting a parent in the editor makes its children jump, and leaves them pointing at a corpse.** 🗑 Delete is `world.despawn(sel)`, which is storage-level only — it does not touch `Parent` / `Children`, and the engine has **no recursive despawn at all** (`src/hierarchy.rs` exports `attach` / `detach` / `reparent` and nothing that removes a subtree). The children keep `Parent(<dead>)`. `HierarchySystem` survives it — `sort_into` treats an unresolvable parent as a root — but the composition arm then falls through to `GlobalTransform::from_transform(local)`, so a child at local `(16, 0)` under a parent at `(500, 300)` stops drawing at `(516, 300)` and starts drawing at `(16, 0)`. Scene save notices separately and logs "parent link dropped". ⚠️ **This is a decision, not a bug to patch**: cascade-delete the subtree, reparent to root preserving world position, or keep today's jump. Nobody would pick the third deliberately, which is why it is filed rather than left. | `src/app/editor/ui/docked/entities_tab.rs:45`, `src/hierarchy.rs:295` | The decision first. Then a unit test — `App::new()` is headless and `HierarchySystem` runs without a GPU, so parent-delete → child `GlobalTransform` is directly assertable |
