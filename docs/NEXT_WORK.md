...
so **the behaviour half of this review is empty**. ⚠️ The v0.159.2 decision left one new row
behind on purpose: nothing but a mode switch writes the settings file, so quitting with the editor
open still saves nothing. It is filed in the table below rather than folded into the closed row.
...
| **(new, 2026-09-10) Quitting with the editor still open saves nothing** — `set_editor_mode` is the only caller of `save_editor_settings`, so a session that closes the window without switching mode drops every preference it changed. This is the deliberate remainder of the v0.159.2 decision, not an oversight: the maintainer chose to widen the transition and leave this. | `src/app/editor/settings.rs:72` (sole writer), `:99` | `quitting_while_the_editor_is_still_open_saves_nothing` already pins today's behaviour and reddens against a save-on-exit, so the row is instrumented before it is scheduled. The fix is a debounced save when `EditorSettings::from_state` differs from what was last written (`RtDebounce` is the precedent); the cost is a per-frame struct compare on the editor path only. Not scheduled — it needs a second complaint or a decision, not a measurement. |
